...
exports.getPoints = async function (req, res) {
  // query string
  // 1. keyword: 키워드 기반 포인트 조회에 이용됨.
  // 2. order: 정렬 선택 시, '추천순', '최신순' 정렬
  // 3. pageId: 페이징 처리 때 페이지 아이디
  let { keyword, order, pageId } = req.query;
...
exports.putPoint = async function (req, res) {
  ...
  // }
  const editPointInfo = await pointService.editPoint(
    pointId,
    title,
    content,
    point_type,
  ...
  if (req.files.length > 0) {
    ...
    pointService.deletePointImg(pointId).then(() => {
      req.files.map((item) => {
        pointService.editPointImg(pointId, item.location);
      });
    });
  }
